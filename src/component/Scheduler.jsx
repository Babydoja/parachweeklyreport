...
// export default function TutorScheduler() {
//   const [people, setPeople] = useState([]);
//   const [slotAssignments, setSlotAssignments] = useState(
//     Array(hours.length).fill(null).map(() => [])
//   );
...
//   const toggleTutorSlot = (tutorId, slotIndex) => {
//     setSlotAssignments(prev => {
//       const slot = prev[slotIndex];
//       const alreadyAssigned = slot.includes(tutorId);

//       if (alreadyAssigned) {
//         return prev.map((s, i) =>
//           i === slotIndex ? s.filter(id => id !== tutorId) : s
//         );
//       } else if (slot.length < chairs) {
//         return prev.map((s, i) =>
//           i === slotIndex ? [...s, tutorId] : s
//         );
//       }

//       return prev; // Do nothing if slot is full
//     });
//   };
...
//   const saveTutor = () => {
//     if (!newTutorName.trim()) return;

//     if (editingTutor) {
//       setPeople(prev =>
//         prev.map(p =>
//           p.id === editingTutor.id ? { ...p, name: newTutorName.trim() } : p
//         )
//       );
//     } else {
//       const id = Date.now();
//       setPeople(prev => [
//         ...prev,
//         { id, name: newTutorName.trim(), color: getRandomColor() },
//       ]);
//     }
...
//     setSlotAssignments(prev =>
//       prev.map(slot => slot.filter(tid => tid !== id))
//     );
//   };

//   return (
//     <div className="p-6 font-[inter] max-w-full overflow-x-auto">
//       <div className="flex justify-between items-center mb-4">
//         <h1 className="text-2xl font-bold">Tutor Scheduler</h1>
//         <button
//           onClick={openAddModal}
//           className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700"
//         >
...
//       <div className="mb-4">
//         <label className="font-medium mr-2">Chairs per time slot:</label>
//         <input
//           type="number"
//           min="1"
...
//                 const assigned = slotAssignments[hourIndex].includes(person.id);
//                 return (
//                   <td
//                     key={hourIndex}
//                     onClick={() => toggleTutorSlot(person.id, hourIndex)}
//                     className={`border px-2 py-1 cursor-pointer text-center transition-colors`}
//                     style={{
//                       backgroundColor: assigned ? person.color : 'white',
//                       color: assigned ? 'white' : 'black',
//                     }}
//                   >
//                     {assigned ? '✔' : ''}
//                   </td>
